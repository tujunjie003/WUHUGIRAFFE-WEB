...
const AGE_GROUPS = [
  ...
  { label: '4-6岁', desc: '语言爆发期', color: 'bg-orange-500/10 border-orange-500/20' },
...
export default function CoursesPage() {
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      {/* 年龄段 */}
      <section className="pb-20 bg-stone-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {AGE_GROUPS.map((group, i) => (
              <Card key={i} className={`p-6 border ${group.color} text-center`}>
                <div className="text-2xl font-semibold text-stone-900 mb-1">{group.label}</div>
                <div className="text-sm text-stone-600">{group.desc}</div>
              </Card>
            ))}
          </div>
        </div>
                  ...
                  <div className="flex-1 p-6 lg:p-8">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {course.highlights.map((h, j) => (
                        <span key={j} className="inline-flex items-center gap-1.5 text-sm text-stone-500">
